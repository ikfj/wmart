...
import org.umart.cmdClientSA.*;
import org.umart.cmdCore.*;
import org.umart.logger.*;
import org.umart.strategy.*;
import org.umart.strategyCore.*;

/**
 * 取引所全体の管理を行うクラスです． ネットワーク環境およびスタンドアロン環境に共通の属性およびメソッドが定義されています．
...
public abstract class UMart {
	 ...
	 *            一日あたりの節数
	 */
	public UMart(UMemberList members, UPriceInfoDB priceInfoDB, int startPoint, long seed, int maxDate,
	...
	/**
	 * ローカルホストで動作するエージェントを登録する．<br>
	 * 
	 * @param agent
	 *            UBaseStrategyを継承したエージェント
	 */
	 ...
	 *            取引所の口座
	 */
	protected void setExchangeBalanceToHashMap(HashMap hash, UExchangeAccount ea) {
	 ...
	 *            収支情報
	 */
	protected void setBalanceToHashMap(HashMap hash, UBalance balance) {
		balance.updateCash();
		hash.put(UCBalancesCore.LONG_CASH, new Long(balance.getCash() + balance.getMargin()));
		hash.put(UCBalancesCore.LONG_MARGIN, new Long(balance.getMargin()));
		hash.put(UCBalancesCore.LONG_UNREALIZED_PROFIT, new Long(balance.getUnrealizedProfit()));
	...
	/**
	 * Balancesコマンドへの応答を処理する。 注文受付時間帯, 取引後時間帯, 決済後時間帯以外はエラーを返す。
	 * 
	 * @param todayHash
	 *            当日分の残高照会情報
	 * @param yesterdayHash
	 ...
	 *            ユーザID
	 */
	public UCommandStatus doBalances(HashMap todayHash, HashMap yesterdayHash, int userID) {
		UCommandStatus ucs = new UCommandStatus();
		try {
			fStateLock.readLock();
			if (!fCmdExecutableChecker.isExecutable(UCBalancesCore.CMD_NAME, fStatus, userID)) {
				ucs.setStatus(false);
			...
			if (account == null) {
				System.err.println("Account not found in UMart.doBalances: UserID=" + userID);
				throw new Exception();
				// System.exit( -5);
				// return ucs;
			}
	...
	public UCommandStatus doAllBalances(ArrayList arrayList, int userID) {
		...
		try {
			fStateLock.readLock();
			if (!fCmdExecutableChecker.isExecutable(UCAllBalancesCore.CMD_NAME, fStatus, userID)) {
				ucs.setStatus(false);
				ucs.setErrorCode(ICommand.UNACCEPTABLE_COMMAND);
				ucs.setErrorMessage(fCmdExecutableChecker.getErrorMessage());
				return ucs;
	...
	/**
	 * AllPositionsコマンドへの応答を処理する。 注文受付時間帯, 取引後時間帯, 決済後時間帯以外はエラーを返す。
	 * 
	 * @param arrayList
	 *            ハッシュマップを格納するためのリスト
	 * @param userID
	...
	public UCommandStatus doAllPositions(ArrayList arrayList, int userID) {
		...
		try {
			fStateLock.readLock();
			if (!fCmdExecutableChecker.isExecutable(UCAllPositionsCore.CMD_NAME, fStatus, userID)) {
				ucs.setStatus(false);
				ucs.setErrorCode(ICommand.UNACCEPTABLE_COMMAND);
				ucs.setErrorMessage(fCmdExecutableChecker.getErrorMessage());
				return ucs;
	...
	/**
	 * 一般ユーザーの収支情報をhashに格納する．UMart.doMemberProfileから呼ばれる．
	 * 
	 * @param hash
	 ...
	 *            一般ユーザーの収支情報
	 */
	protected void setBalanceToHashMapForMemberProfile(HashMap hash, UBalance bal) {
		...
		hash.put(UCMemberProfileCore.LONG_UNREALIZED_PROFIT, new Long(unrealizedProfit)); // 未実現損益
	...
	/**
	 * 一般ユーザーのポジション情報をhashに格納する．UMart.doMemberProfileから呼ばれる．
	 * 
	 * @param hash
	 ...
	 *            一般ユーザーのポジション情報
	 */
	protected void setPositionToHashMapForMemberProfile(HashMap hash, UPosition pos) {
		long sumOfSellPositionsUntilYesterday = pos.getSumOfSellPositionUntilYesterday();
		hash.put(UCMemberProfileCore.LONG_SUM_OF_SELL_POSITIONS_UNTIL_YESTERDAY, new Long(
		...
		hash.put(UCMemberProfileCore.LONG_SUM_OF_BUY_POSITIONS_UNTIL_YESTERDAY, new Long(
			sumOfBuyPositionsUntilYesterday)); // 前日までの買ポジションの合計
		long todaySellPositions = pos.getSumOfTodaySellPosition();
		hash.put(UCMemberProfileCore.LONG_TODAY_SELL_POSITIONS, new Long(todaySellPositions)); // 当日の売ポジションの合計
		long todayBuyPositions = pos.getSumOfTodayBuyPosition();
		hash.put(UCMemberProfileCore.LONG_TODAY_BUY_POSITIONS, new Long(todayBuyPositions)); // 当日の買ポジションの合計
	}
	 ...
	 * @return 実行状態
	 */
	public UCommandStatus doMemberProfile(HashMap data, int userID, int targetUserId) {
		UCommandStatus ucs = new UCommandStatus();
		try {
			fStateLock.readLock();
			if (!fCmdExecutableChecker.isExecutable(UCMemberProfileCore.CMD_NAME, fStatus, userID)) {
				ucs.setStatus(false);
				ucs.setErrorCode(ICommand.UNACCEPTABLE_COMMAND);
				ucs.setErrorMessage(fCmdExecutableChecker.getErrorMessage());
				return ucs;
			...
			data.put(UCMemberProfileCore.ARRAY_LIST_SYSTEM_PARAMETERS, account.getSystemParameters()); // システムパラメータ
			data.put(UCMemberProfileCore.INT_SEED, new Integer(account.getSeed())); // 乱数の種
			data.put(UCMemberProfileCore.LONG_INITIAL_CASH, new Long(account.getInitialCash())); // 初期資産
			data.put(UCMemberProfileCore.LONG_TRADING_UNIT, new Long(account.getTradingUnit())); // 取引単位
			data.put(UCMemberProfileCore.LONG_FEE_PER_UNIT, new Long(account.getFeePerUnit())); // 単位取引あたりの手数料
			data.put(UCMemberProfileCore.LONG_MARGIN_RATE, new Long(account.getMarginRate())); // 証拠金率
			data.put(UCMemberProfileCore.LONG_MAX_LOAN, new Long(account.getMaxLoan())); // 借り入れ限度額
			data.put(UCMemberProfileCore.DOUBLE_INTEREST, new Double(account.getInterestPerYear())); // 借り入れ金利
			data.put(UCMemberProfileCore.INT_STATUS, new Integer(account.getStatus())); // 取引可能(+1),取引不可能(-1)
			data.put(UCMemberProfileCore.INT_NO_OF_LOGIN_AGENTS, new Integer(office.getNoOfLoginAgents())); // ログイン中のエージェント数
			...
			data.put(UCMemberProfileCore.HASHMAP_YESTERDAY_BALANCE, yesterdayBalanceHash); // 前日収支
	...
	/**
	 * ExchangeProfileコマンドを処理する．
	 * 
	 * @param data
	 *            取引所情報を格納するためのHashMap
	 * @param userID
	 *            このコマンドを実行したユーザーのユーザーID
	 * @return 実行状態
	 */
	public UCommandStatus doExchangeProfile(HashMap data, int userID) {
		...
		try {
			fStateLock.readLock();
			if (!fCmdExecutableChecker.isExecutable(UCExchangeProfileCore.CMD_NAME, fStatus, userID)) {
				ucs.setStatus(false);
				ucs.setErrorCode(ICommand.UNACCEPTABLE_COMMAND);
				ucs.setErrorMessage(fCmdExecutableChecker.getErrorMessage());
				return ucs;
	...
	/**
	 * BoardDataコマンド(過去の板寄せを行った直後の板を得るためのコマンド)への応答を処理する。 注文受付時間帯, 板寄せ後時間帯，取引後時間帯以外はエラーを返す。 ただし, 1日目の１回目の注文期間はエラーを返す。
	 * 
	 * @param hash
	 *            各情報を格納するためのハッシュマップ
	 * @param arrayList
	...
	public UCommandStatus doBoardData(HashMap hash, ArrayList arrayList) {
		...
		try {
			fStateLock.readLock();
			if (!fCmdExecutableChecker.isExecutable(UCBoardDataCore.CMD_NAME, fStatus, UMart.SU_ID)) {
				ucs.setStatus(false);
				ucs.setErrorCode(ICommand.UNACCEPTABLE_COMMAND);
				ucs.setErrorMessage(fCmdExecutableChecker.getErrorMessage());
				return ucs;
			...
			ucs.setStatus(true);
			hash.put(UCBoardDataCore.LONG_TOTAL_BUY_VOLUME, new Long(fBoardInformation.getTotalBuyVolume()));
			hash.put(UCBoardDataCore.LONG_MIN_PRICE, new Long(fBoardInformation.getMinPrice()));
			hash.put(UCBoardDataCore.LONG_MAX_PRICE, new Long(fBoardInformation.getMaxPrice()));
			hash.put(UCBoardDataCore.LONG_CONTRACT_PRICE, new Long(fBoardInformation.getContractPrice()));
			hash.put(UCBoardDataCore.LONG_CONTRACT_VOLUME, new Long(fBoardInformation.getContractVolume()));
			...
			if (info.getSellVolume() > 0) {
				...
				marketSell.put(UCBoardDataCore.LONG_PRICE, new Long(fBoardInformation.getMinPrice()));
			...
			while (elements.hasMoreElements()) {
				...
				if (info.getSellVolume() > 0) {
					...
					limitSell.put(UCBoardDataCore.LONG_VOLUME, new Long(info.getSellVolume()));
			...
			if (marketBuyVolume > 0) {
				...
				marketBuy.put(UCBoardDataCore.LONG_PRICE, new Long(fBoardInformation.getMaxPrice()));
	...
	/**
	 * BoardInformationコマンド（最新の板情報を得るためのコマンド）への応答を処理する。 注文受付時間帯, 板寄せ後時間帯，取引後時間帯以外はエラーを返す。 ただし, 1日目の１回目の注文期間はエラーを返す。
	 * 
	 * @param data
	 *            板の最終更新時刻，成行注文数量，各価格ごとの注文数量
	 */
	public UCommandStatus doBoardInformation(HashMap data) {
		UCommandStatus ucs = new UCommandStatus();
		try {
			fStateLock.readLock();
			if (!fCmdExecutableChecker.isExecutable(UCBoardInformationCore.CMD_NAME, fStatus, SU_ID)) {
				ucs.setStatus(false);
				ucs.setErrorCode(ICommand.UNACCEPTABLE_COMMAND);
				ucs.setErrorMessage(fCmdExecutableChecker.getErrorMessage());
				return ucs;
			...
			while (elements.hasMoreElements()) {
				...
				hash.put(UCBoardInformationCore.LONG_SELL_VOLUME, new Long(info.getSellVolume()));
				hash.put(UCBoardInformationCore.LONG_BUY_VOLUME, new Long(info.getBuyVolume()));
				arrayList.add(hash);
			}
	 ...
	 *            実時間
	 */
	protected String extractTime(Date date) {
		return new SimpleDateFormat("HH:mm:ss").format(date);
	}

	/**
	 * OrderCancelコマンドへの応答を処理する。 注文受付時間帯以外はエラーを返す。
	 * 
	 * @param hash
	 *            各情報を格納するためのハッシュマップ
	 * @param userID
	...
	public UCommandStatus doOrderCancel(HashMap hash, int userID, long orderID) {
		...
		try {
			fStateLock.readLock();
			if (!fCmdExecutableChecker.isExecutable(UCOrderCancelCore.CMD_NAME, fStatus, userID)) {
				ucs.setStatus(false);
				ucs.setErrorCode(ICommand.UNACCEPTABLE_COMMAND);
				ucs.setErrorMessage(fCmdExecutableChecker.getErrorMessage());
				return ucs;
			...
			synchronized (fOrderManager) {
				...
				if (o != null) {
					...
					hash.put(UCOrderCancelCore.LONG_CANCEL_VOLUME, Long.toString(uncontractVolume));
					hash.put(UCOrderCancelCore.LONG_UNCANCEL_VOLUME, Long.toString(contractVolume));
					fOrderCommandLog.registerOrderCancel(fStatus.getDate(), fStatus.getSession(), o.getUserName(),
						orderID, uncontractVolume);
				} else {
					ucs.setStatus(false);
					ucs.setErrorCode(ICommand.INVALID_ARGUMENTS);
					ucs.setErrorMessage("THE ORDER WITH ID:" + orderID + " CANNOT BE FOUND");
				}
			}
		} catch (Exception e) {
	...
	public UCommandStatus doOrderCheck(long orderID, HashMap hash) {
		...
		try {
			fStateLock.readLock();
			if (!fCmdExecutableChecker.isExecutable(UCOrderCheckCore.CMD_NAME, fStatus, SU_ID)) {
				ucs.setStatus(false);
				ucs.setErrorCode(ICommand.UNACCEPTABLE_COMMAND);
				ucs.setErrorMessage(fCmdExecutableChecker.getErrorMessage());
				return ucs;
			...
			synchronized (o) {
				...
				hash.put(UCOrderCheckCore.INT_MARKET_LIMIT, new Integer(o.getMarketLimit()));
				...
				hash.put(UCOrderCheckCore.LONG_SUM_OF_CONTRACT_VOLUME, new Long(o.getContractVolume()));
				hash.put(UCOrderCheckCore.LONG_CANCEL_VOLUME, new Long(o.getCancelVolume()));
				ArrayList contractInfoArray = new ArrayList();
				Enumeration enumeration = o.getContractInformationArray().elements();
				while (enumeration.hasMoreElements()) {
					UContractInformation info = (UContractInformation) enumeration.nextElement();
					HashMap infoHash = new HashMap();
					infoHash.put(UCOrderCheckCore.LONG_CONTRACT_ID, new Long(info.getContractID()));
					infoHash.put(UCOrderCheckCore.STRING_CONTRACT_TIME, this.extractTime(info.getTime()));
					infoHash.put(UCOrderCheckCore.LONG_CONTRACT_PRICE, new Long(info.getPrice()));
					infoHash.put(UCOrderCheckCore.LONG_CONTRACT_VOLUME, new Long(info.getVolume()));
					infoHash.put(UCOrderCheckCore.INT_CONTRACT_DATE, new Integer(info.getDate()));
					infoHash.put(UCOrderCheckCore.INT_CONTRACT_BOARD_NO, new Integer(info.getSession()));
					contractInfoArray.add(infoHash);
				}
				hash.put(UCOrderCheckCore.ARRAYLIST_CONTRACT_INFORMATION_ARRAY, contractInfoArray);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.exit(5);
	...
	/**
	 * OrderRequestコマンドへの応答を処理する。 注文受付時間帯以外はエラーを返す。
	 * 
	 * @param hash
	 *            各情報を格納するためのハッシュマップ
	 * @param userID
	 ...
	 *            希望取引数量
	 */
	public UCommandStatus doOrderRequest(HashMap hash, int userID, String brandName, int newRepay, int sellBuy,
		int marketLimit, long price, long volume) {
		UCommandStatus ucs = new UCommandStatus();
		try {
			fStateLock.readLock();
			if (!fCmdExecutableChecker.isExecutable(UCOrderRequestCore.CMD_NAME, fStatus, userID)) {
				ucs.setStatus(false);
				ucs.setErrorCode(ICommand.UNACCEPTABLE_COMMAND);
				ucs.setErrorMessage(fCmdExecutableChecker.getErrorMessage());
				return ucs;
			...
			UOrder o = fOrderManager.createOrder(userID, userName, brandName, sellBuy, marketLimit, newRepay, price,
				volume, date, boardNo);
			synchronized (fOrderManager) {
	...
	/**
	 * TodaysQuotesコマンドへの応答を処理する。 注文受付時間帯, 取引後時間帯, 決裁後時間帯以外はエラーを返す。
	 * 
	 * @param arrayList
	 *            ハッシュマップを格納するためのリスト
	 * @param brandName
	 ...
	 *            過去板寄せ何回分のの情報が必要か?
	 */
	public UCommandStatus doTodaysQuotes(ArrayList arrayList, String brandName, int noOfBoards) {
		// noOfBoards < 0 で全ての板寄せについての情報を返す
		UCommandStatus ucs = new UCommandStatus();
		try {
			fStateLock.readLock();
			if (!fCmdExecutableChecker.isExecutable(UCTodaysQuotesCore.CMD_NAME, fStatus, SU_ID)) {
				ucs.setStatus(false);
				ucs.setErrorCode(ICommand.UNACCEPTABLE_COMMAND);
				ucs.setErrorMessage(fCmdExecutableChecker.getErrorMessage());
				return ucs;
			...
			for (int i = startIndex; i >= endIndex; --i) {
				...
				hash.put(UCTodaysQuotesCore.LONG_ASKED_QUOTATION, new Long(tq.getAskedQuotation()));
				hash.put(UCTodaysQuotesCore.LONG_BID_QUOTATION, new Long(tq.getBidQuotation()));
				arrayList.add(hash);
			}
		} catch (Exception e) {
	...
	/**
	 * ハッシュマップhashに注文情報oの各データを格納する。<br>
	 * 
	 * @param hash
	 *            情報を格納するためのハッシュマップ
	 * @param o
	 *            注文
	 */
	protected void sendOrderMessage(HashMap hash, UOrder o) {
		...
		hash.put(UCOrderStatusCore.INT_MARKET_LIMIT, Integer.toString(o.getMarketLimit()));
		...
		hash.put(UCOrderStatusCore.LONG_CONTRACTED_VOLUME, Long.toString(o.getContractVolume()));
		hash.put(UCOrderStatusCore.LONG_CANCEL_VOLUME, Long.toString(o.getCancelVolume()));
	}

	/**
	 * OrderStatusコマンドへの応答を処理する。 注文受付時間帯, 取引後時間帯以外はエラーを返す。
	 * 
	 * @param arrayList
	 *            ハッシュマップを格納するためのリスト
	 * @param userID
	...
	public UCommandStatus doOrderStatus(ArrayList arrayList, int userID) {
		...
		try {
			fStateLock.readLock();
			if (!fCmdExecutableChecker.isExecutable(UCOrderStatusCore.CMD_NAME, fStatus, userID)) {
				ucs.setStatus(false);
				ucs.setErrorCode(ICommand.UNACCEPTABLE_COMMAND);
				ucs.setErrorMessage(fCmdExecutableChecker.getErrorMessage());
				return ucs;
	 ...
	 *            約定情報
	 */
	protected void setContractInfoToHash(HashMap hash, UOrder o, UContractInformation info) {
	...
	/**
	 * Executionsコマンドへの応答を処理する。 注文受付時間帯, 取引後時間帯以外はエラーを返す。
	 * 
	 * @param arrayList
	 *            ハッシュマップを格納するためのリスト
	 * @param userID
	...
	public UCommandStatus doExecutions(ArrayList arrayList, int userID) {
		...
		try {
			fStateLock.readLock();
			if (!fCmdExecutableChecker.isExecutable(UCExecutionsCore.CMD_NAME, fStatus, userID)) {
				ucs.setStatus(false);
				ucs.setErrorCode(ICommand.UNACCEPTABLE_COMMAND);
				ucs.setErrorMessage(fCmdExecutableChecker.getErrorMessage());
				return ucs;
	...
	/**
	 * FuturePriceコマンドへの応答を処理する。 取引前時間帯, 注文受付時間帯, 取引後時間帯, 決済後時間帯以外は エラーを返す。
	 * 
	 * @param arrayList
	 *            ハッシュマップを格納するためのリスト
	 * @param brandName
	 ...
	 *            過去何ステップ分の先物価格が必要か?
	 */
	public UCommandStatus doFuturePrice(ArrayList arrayList, String brandName, int noOfSteps) {
		UCommandStatus ucs = new UCommandStatus();
		try {
			fStateLock.readLock();
			if (!fCmdExecutableChecker.isExecutable(UCFuturePriceCore.CMD_NAME, fStatus, SU_ID)) {
				ucs.setStatus(false);
				ucs.setErrorCode(ICommand.UNACCEPTABLE_COMMAND);
				ucs.setErrorMessage(fCmdExecutableChecker.getErrorMessage());
				return ucs;
	...
	/**
	 * HistoricalQuotesコマンドへの応答を処理する。 注文受付時間帯, 取引後時間帯, 決裁後時間帯以外はエラーを返す。
	 * 
	 * @param arrayList
	 *            ハッシュマップを格納するためのリスト
	 * @param brandName
	 ...
	 *            過去何日分の情報が必要か?
	 */
	public UCommandStatus doHistoricalQuotes(ArrayList arrayList, String brandName, int noOfDays) {
		// noOfDays < 0 で過去全てを返す
		UCommandStatus ucs = new UCommandStatus();
		try {
			fStateLock.readLock();
			if (!fCmdExecutableChecker.isExecutable(UCHistoricalQuotesCore.CMD_NAME, fStatus, SU_ID)) {
				ucs.setStatus(false);
				ucs.setErrorCode(ICommand.UNACCEPTABLE_COMMAND);
				ucs.setErrorMessage(fCmdExecutableChecker.getErrorMessage());
				return ucs;
			...
			for (int i = startIndex; i >= endIndex; --i) {
				...
				hash.put(UCHistoricalQuotesCore.LONG_START_PRICE, new Long(hq.getStartPrice()));
				hash.put(UCHistoricalQuotesCore.LONG_HIGHEST_PRICE, new Long(hq.getHighestPrice()));
				hash.put(UCHistoricalQuotesCore.LONG_LOWEST_PRICE, new Long(hq.getLowestPrice()));
			...
			System.err.println("Exception: " + e + " in UMart.doHistoricalQuotes");
			System.exit(5);
		} finally {
			fStateLock.readUnlock();
			return ucs;
	...
	public UCommandStatus doMarketStatus(HashMap hash) {
		...
		try {
			fStateLock.readLock();
			ucs.setStatus(true);
			hash.put(UCMarketStatusCore.INT_MARKET_STATUS, new Integer(fStatus.getState()));
		} catch (Exception e) {
	...
	/**
	 * OrderIDHistoryコマンドへの応答を処理する．
	 * 
	 * @param selfUserID
	 *            このコマンドを実行したユーザーのID
	 * @param arrayList
	 *            指定されたユーザーIDの過去noOfSteps分の注文依頼のIDを格納するためのArrayList
	 * @param targetUserID
	 *            指定ユーザーID (-1の場合，selfUserIDが代入される）
	 * @param noOfSteps
	 *            過去板寄せ何回分の情報がほしいか？
	 * @return コマンドの処理結果
	 */
	public UCommandStatus doOrderIDHistory(int selfUserID, ArrayList arrayList, int targetUserID, int noOfSteps) {
		arrayList.clear();
		UCommandStatus ucs = new UCommandStatus();
		try {
			fStateLock.readLock();
			...
			if ((selfUserID != targetUserID && selfUserID != UMart.SU_ID)
				|| !fCmdExecutableChecker.isExecutable(UCOrderIDHistoryCore.CMD_NAME, fStatus, targetUserID)) {
				ucs.setStatus(false);
				ucs.setErrorCode(ICommand.UNACCEPTABLE_COMMAND);
				ucs.setErrorMessage(fCmdExecutableChecker.getErrorMessage());
				return ucs;
			}
			int sumOfSteps = (fStatus.getDate() - 1) * fNoOfSessionsPerDay + fStatus.getSession();
	...
	/**
	 * Positionコマンドへの応答を処理する。 注文受付時間帯, 取引後時間帯, 決済後時間帯以外はエラーを返す。
	 * 
	 * @param hash
	 *            各情報を格納するためのハッシュマップ
	 * @param userID
	...
	public UCommandStatus doPosition(HashMap hash, int userID) {
		...
		try {
			fStateLock.readLock();
			if (!fCmdExecutableChecker.isExecutable(UCPositionCore.CMD_NAME, fStatus, userID)) {
				ucs.setStatus(false);
				ucs.setErrorCode(ICommand.UNACCEPTABLE_COMMAND);
				ucs.setErrorMessage(fCmdExecutableChecker.getErrorMessage());
				return ucs;
	...
	/**
	 * SetSpotDateコマンドへの応答を処理する。 取引前時間帯以外はエラーを返す。
	 * 
	 * @param userID
	 *            ユーザID
	 * @param startPoint
	...
	public UCommandStatus doSetSpotDate(int userID, int startPoint) {
		...
		try {
			fStateLock.readLock();
			if (!fCmdExecutableChecker.isExecutable(UCSetSpotDateCore.CMD_NAME, fStatus, userID)) {
				ucs.setStatus(false);
				ucs.setErrorCode(ICommand.UNACCEPTABLE_COMMAND);
				ucs.setErrorMessage(fCmdExecutableChecker.getErrorMessage());
				return ucs;
			...
			System.err.println("Exception:SetSchedule " + e + " in UMart.doSetSpotDate");
			System.exit(5);
		} finally {
			fStateLock.readUnlock();
			return ucs;
	...
	public UCommandStatus doSpotInterval(HashMap hash) {
		...
		try {
			fStateLock.readLock();
			if (!fCmdExecutableChecker.isExecutable(UCSpotIntervalCore.CMD_NAME, fStatus, SU_ID)) {
				ucs.setStatus(false);
				ucs.setErrorCode(ICommand.UNACCEPTABLE_COMMAND);
				ucs.setErrorMessage(fCmdExecutableChecker.getErrorMessage());
				return ucs;
			...
			ucs.setStatus(true);
			hash.put(UCSpotIntervalCore.INT_SPOT_INTERVAL, new Integer(UMart.DEFAULT_SPOT_INTERVAL));
			return ucs;
		} catch (Exception e) {
	...
	/**
	 * SpotPriceコマンドへの応答を処理する。 取引前時間帯, 注文受付時間帯, 取引後時間帯, 決済後時間帯以外は エラーを返す。
	 * 
	 * @param arrayList
	 *            ハッシュマップを格納するためのリスト．先頭の要素は直近の価格情報．
	 * @param brandName
	 *            銘柄名
	 * @param noOfSteps
	 *            過去何ステップ分の現物価格が必要か?
	 */
	public UCommandStatus doSpotPrice(ArrayList arrayList, String brandName, int noOfSteps) {
		UCommandStatus ucs = new UCommandStatus();
		try {
			fStateLock.readLock();
			if (!fCmdExecutableChecker.isExecutable(UCSpotPriceCore.CMD_NAME, fStatus, SU_ID)) {
				ucs.setStatus(false);
				ucs.setErrorCode(ICommand.UNACCEPTABLE_COMMAND);
				ucs.setErrorMessage(fCmdExecutableChecker.getErrorMessage());
				return ucs;
	...
	public UCommandStatus doServerStatus(HashMap data, int userID) {
		...
		try {
			fStateLock.readLock();
			if (!fCmdExecutableChecker.isExecutable(UCServerStatusCore.CMD_NAME, fStatus, userID)) {
				ucs.setStatus(false);
				ucs.setErrorCode(ICommand.UNACCEPTABLE_COMMAND);
				ucs.setErrorMessage(fCmdExecutableChecker.getErrorMessage());
				return ucs;
	...
	/**
	 * AccountHistoryコマンドを処理する．
	 * 
	 * @param array
	 *            口座履歴要素(HashMap)が格納される配列
	 * @param userID
	 *            このコマンドを実行したユーザーのユーザーID
	 * @param targetUserId
	 *            このユーザーID（-1の場合，userIDが代入される）をもつユーザーの口座履歴をarrayに格納する
	 * @param noOfSteps
	 *            指定されたステップ数分の直近の口座情報を返す
	 * @return 実行状態
	 */
	public UCommandStatus doAccountHistory(ArrayList array, int userID, int targetUserId, long noOfSteps) {
		UCommandStatus ucs = new UCommandStatus();
		try {
			fStateLock.readLock();
			if (!fCmdExecutableChecker.isExecutable(UCAccountHistoryCore.CMD_NAME, fStatus, userID)) {
				ucs.setStatus(false);
				ucs.setErrorCode(ICommand.UNACCEPTABLE_COMMAND);
				ucs.setErrorMessage(fCmdExecutableChecker.getErrorMessage());
				return ucs;
			...
			ArrayList history = fAccountManager.getAccount(targetUserId).getAccountHistory();
			if (noOfSteps < 0) {
				for (int i = history.size() - 1; i >= 0; --i) {
	 ...
	 * ：<br>
	 * ：<br>
	 * 板寄せn，注文受付(UServerStatus.ACCEPT_ORDERS) 板寄せn, 板寄せ(UServerStatus.ITAYOSE) ・・・内部状態 板寄せn,
	 * 板寄せ後(UServerStatus.AFTER_ITAYOSE)<br>
		...
		try {
			...
			case UServerStatus.ACCEPT_ORDERS:
				...
				if (fStatus.getSession() == fNoOfSessionsPerDay) {
					...
					markToMarket();
					fAccountManager.backupAllBalances((fStatus.getDate() - 1) * fNoOfSessionsPerDay
						+ fStatus.getSession());
	...
	/**
	 * 取引所の状態を「板寄せ時間帯」に設定し, 板寄せ処理を行う。 具体的な手順は以下のとおり：<br>
	...
	protected void makeContracts() {
		...
		fQuoteDB.addTodaysQuote("j30", date, boardNo, price, volume, askedQuotation, bidQuotation);
	...
	/**
	 * UOrderManagerの未約定注文配列(UncontractedOrderArray)から 全注文(UOrder)の参照を外す．<br>
	 */
	protected void removeUncontractedOrdersFromOrderManager() {
	...
	/**
	 * 現在の板の形状(売注文と買い注文の情報)をUBoardInformationに保存する．<br>
	 */
	protected void saveBoardShape() {
		fBoardInformation.clear();
		Iterator orders = fBoard.getOrderArray();
		while (orders.hasNext()) {
			UOrder o = (UOrder) orders.next();
			fBoardInformation.addInformation(o.getSellBuy(), o.getMarketLimit(), o.getPrice(), o
				.getUncontractOrderVolume());
		}
	}

	/**
	 * UAccountManagerへ全約定情報を登録する。
	 */
	protected void registerContractsToAccountManager() {
		Iterator orders = fBoard.getOrderArray();
		while (orders.hasNext()) {
			UOrder o = (UOrder) orders.next();
			...
			while (infos.hasMoreElements()) {
				...
				if (info.getSession() == fStatus.getSession()) {
					...
					if (!fAccountManager.registerContract(userID, sellBuy, volume, fStatus.getSession())) {
	 ...
	 *            約定数量
	 */
	protected void updateBoardInformation(long contractPrice, long contractVolume) {
	...
	/**
	 * 全注文(UOrder)をUBoardからUOrderManagerへ移動する。
	 */
	protected void moveOrdersFromBoardToOrderManager() {
		Iterator orders = fBoard.getOrderArray();
		while (orders.hasNext()) {
			UOrder o = (UOrder) orders.next();
			fOrderManager.addOrder(o);
	 ...
	 * 破産した会員の注文を全てキャンセルする。
	 */
	protected void cancelOrdersOfBankruptedMembers() {
	...
	/**
	 * UOrderManager中のすべての未約定注文をUBoardに登録する．
	 */
	protected void registerUncontractedOrdersToBoard() {
		Enumeration arrays = fOrderManager.getOrderArrays();
		while (arrays.hasMoreElements()) {
			UOrderArray orderArray = (UOrderArray) arrays.nextElement();
			Vector uncontractedOrderArray = orderArray.getUncontractedOrders();
	 ...
	 * @throws IOException
	 */
	protected void updatePriceInfoDBFile(String path) throws IOException {
	 ...
	 * @throws IOException
	 */
	protected void writeOrderCommandLog(String path) throws IOException {
	 ...
	 * @throws IOException
	 */
	protected void writeExecutionLog(String path, int date, int session, UOrderManager orderManager) throws IOException {
		PrintWriter pw = new PrintWriter(new FileOutputStream(path));
	 ...
	 *            int 節
	 * @param boardInfo
	 *            UBoardInformation 板情報
	 * @throws IOException
	 */
	protected void writeBoardLog(String path, int date, int session, UBoardInformation boardInfo) throws IOException {
		PrintWriter pw = new PrintWriter(new FileOutputStream(path));
	 ...
	 *            UAccountManager 口座管理者
	 * @throws IOException
	 */
	protected void writeAccountLog(String path, int date, int session, UAccountManager accountManager)
		throws IOException {
		PrintWriter pw = new PrintWriter(new FileOutputStream(path));
	 ...
	 *            マーケット状態
	 */
	protected void updateLog(int date, int session, int state) throws IOException {
		...
		case UServerStatus.ITAYOSE:
			...
			if (fLogFlag == UMart.DETAILED_LOG) {
				writeOrderCommandLog(fLogDir + File.separator + UMart.ORDER_LOG_DIR + File.separator + "Day" + date
					+ "Session" + session + "_" + UMart.ORDER_LOG_DIR + ".csv");
				fOrderCommandLog.clear();
				writeExecutionLog(fLogDir + File.separator + UMart.EXECUTION_LOG_DIR + File.separator + "Day" + date
					+ "Session" + session + "_" + UMart.EXECUTION_LOG_DIR + ".csv", date, session, fOrderManager);
				writeBoardLog(fLogDir + File.separator + UMart.BOARD_LOG_DIR + File.separator + "Day" + date
					+ "Session" + session + "_" + UMart.BOARD_LOG_DIR + ".csv", date, session, fBoardInformation);
				writeAccountLog(fLogDir + File.separator + UMart.ACCOUNT_LOG_DIR + File.separator + "Day" + date
					+ "Session" + session + "_" + UMart.ACCOUNT_LOG_DIR + ".csv", date, session, fAccountManager);
			} else {
				fOrderCommandLog.clear();
			}
			break;
		case UServerStatus.AFTER_MARKING_TO_MARKET:
			if (fLogFlag == UMart.DETAILED_LOG) {
				writeAccountLog(fLogDir + File.separator + UMart.ACCOUNT_LOG_DIR + File.separator + "Day" + date
					+ "_MM" + "_" + UMart.ACCOUNT_LOG_DIR + ".csv", date, session, fAccountManager);
			}
			break;
		case UServerStatus.AFTER_SETTLEMENT:
			updatePriceInfoDBFile(fLogDir + File.separator + "price.csv");
			writeAccountLog(fLogDir + File.separator + "Settlement_" + UMart.ACCOUNT_LOG_DIR + ".csv", date, session,
				fAccountManager);
			if (fLogFlag == UMart.DETAILED_LOG) {
				writeAccountLog(fLogDir + File.separator + UMart.ACCOUNT_LOG_DIR + File.separator + "Settlement_"
					+ UMart.ACCOUNT_LOG_DIR + ".csv", date, session, fAccountManager);
			}
			break;
		default:
			System.err.println("Invalid state:" + state + " in UMartStandAlone.updateLog");
		}
	}

	/**
	 * 取引所の状態を「値洗い時間帯」に設定し, 値洗い処理を行う。
	 */
	protected void markToMarket() {
	...
	/**
	 * 取引所の状態を「決算時間帯」に設定し, 決算処理を行う。 日付は, 最終決算日+1に設定される。<br>
	 */
	...
	/**
	 * 詳細ログの初期化を行う． 具体的には，ログを出力するディレクトリを作成し，price.csv, TimeSeriesDefinitions.csv, Member.csvを作成する．
	 * 
	...
	/**
	 * 出力先ディレクトリを指定して，詳細ログの初期化を行う．
	 * 
	 * @param baseDir
	 *            String 出力先ディレクトリ名
	 * @param simpleFlag
	 *            boolean 簡易ログならばtrue, 詳細ログならばfalse
	 * @throws FileNotFoundException
	 * @throws IOException
	 */
	public void initLog(String baseDir, boolean simpleFlag) throws FileNotFoundException, IOException {
		...
		writeTimeSeriesDefinitionLog(fLogDir + File.separator + "TimeSeriesDefinitions.csv");
	...
	/**
	 * PrinceInfoDBをファイルに書き出す．
	 * 
	 * @param path
	 *            String 出力ファイル名
	 * @throws IOException
	 */
	protected void writePriceInfoDB(String path) throws IOException {
		PrintWriter pw = new PrintWriter(new FileOutputStream(path));
	...
	/**
	 * TimeSeriesDefinitionLogをファイルに書き出す．
	 * 
	 * @param path
	 *            String 出力ファイル名
	 * @throws IOException
	 */
	protected void writeTimeSeriesDefinitionLog(String path) throws IOException {
		PrintWriter pw = new PrintWriter(new FileWriter(path));
		UTimeSeriesDefinitions timeSeriesDefinitionLog = new UTimeSeriesDefinitions();
		timeSeriesDefinitionLog.addDefinition("Default", "price.csv", "j30", fStartPoint
			- (Strategy.NO_OF_SPOT_PRICES - 1), fMaxDate, fNoOfSessionsPerDay);
	 ...
	 * @throws IOException
	 */
	protected void writeMemberLog(String path) throws IOException {
		PrintWriter pw = new PrintWriter(new FileOutputStream(path));
	...
	/**
	 * ログを出力するデフォルトのディレクトリ名を作成して返す．
	 * 
	 * @return String ディレクトリ名（UMART＋年＋月＋日＋時＋分＋秒＋ミリ秒）
	 */
	protected String makeDefaultDirectoryName() {
		String result = "./UMART";
	 ...
	 * 
	 * @param baseDir
	 *            String ログ出力用のディレクトリ名．nullの場合，デフォルトのディレクトリが生成される．
	 * @param simpleFlag
	 *            boolean 簡易ログならばtrue, 詳細ログならばfalse
	 */
	protected boolean makeLogDirectory(String baseDir, boolean simpleFlag) {
		if (baseDir == null) {
	...
	/**
	 * fLogDirで表されるログ出力用ディレクトリの下にサブディレクトリnameを作成する．<br>
	 * 
	 * @param name
	 *            サブディレクトリ名（相対）
	 * @return サブディレクトリの生成に成功した場合：true, 失敗した場合：false
	 */
	protected boolean makeSubDirectory(String name) {
	...
	/**
	 * 口座情報にしたがって, fOrderManagerの初期設定を行う。 したがって, このメソッドを呼び出す前に, fAccountManagerが 正しく初期化されている必要がある。
	 */
	protected void setupOrderManager() {
		Enumeration accounts = fAccountManager.getAccounts();
		while (accounts.hasMoreElements()) {
			UAccount account = (UAccount) accounts.nextElement();
	...
	/**
	 * fLoginMangerの初期化を行う． 具体的には，ログインマネージャーを生成させる． したがって，このメソッドを呼び出す前に，fAccountManagerが正しく 初期化されている必要がある．
	 */
	protected void setupLoginManager() {
	...
	/**
	 * 取引所の状態を「注文受付時間帯」に設定し, エージェントごとに 注文を受けつける。
	 */
