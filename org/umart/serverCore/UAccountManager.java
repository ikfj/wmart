...
public class UAccountManager {
   ...
   * 含む)のユーザーIDを返す。もし, 該当するユーザーが存在しなければ
   * -1を返す。
  ...
  /**
   * 引数で指定されたユーザーIDをもつユーザーの口座を返す。
   * ただし, 指定されたユーザーIDをもつ口座が見つからなければnullを返す。
   * @param id ユーザーID
...
//				account.backup();
  ...
  /**
   * 最終決済処理を行う。
   * @param tommorrowSpotPrice 最終決済価格(最終日翌日の現物価格)
   */
  ...
  /**
   * 約定価格を設定する。registerContractメソッドを呼び出す前に,
   * 必ず一度呼び出す必要がある。
   * @param price 約定価格
   * @param boards 板寄せ回数
   */
  ...
  /**
   * 金利を引数で指定されたユーザーから取引所へ支払う。
   * このメソッドはmarkToMarketメソッド内から呼び出される。
   ...
   * 口座へ移動する。markToMarketメソッドから呼び出される。
   * @param account 対象となる口座
  ...
  /**
   * 引数で指定された口座について取引所からvalueだけ借入れる。
   * ただし, 借入額の合計は借入れ限度額を越えることはできない。
   * @param account 対象となる口座
   * @param value 借入額
   */
  ...
  /**
   * 引数で指定された口座について破産処理を行う。
   * 具体的には, 取引き停止にした後, 保有ポジションを取引所へ移動, 破産
   * したユーザーの口座を格納しているベクタに登録する。
   * @param account 対象となる口座
   */
   ...
   * 必要ならばbankruptメソッドを呼び出すことにより破産処理をおこなう。
   * @param account 対象となる口座
   */
