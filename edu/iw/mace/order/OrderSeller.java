...
/**
 * Represents an order of a seller
 * 
 * Created: 20.08.2004
 * 
 ...
 * @version 1.1
 */
public class OrderSeller extends AbstractOrder {
	 ...
	 *            The seller agent
	 */
	public OrderSeller(Agent agent) throws MaceException {
		super(agent);
	}
	...
	public Element toXML(Document document) {
		Element element = document.createElement("sellerorder");
		element.setAttribute("agent", getAgent().getId());
		for (int x = 0; x < bundleOrders.size(); x++) {
			BundleOrderSeller bundleOrder = (BundleOrderSeller) bundleOrders
					.values().toArray()[x];
